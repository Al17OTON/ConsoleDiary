...
public:
	...
	void insert_diary(diary::diary_data& data);
	...
	diary get_diary_by_id(const int id) const;
	...
	void erase_diary_by_id(const int id);
...
private:
	//set<diary> m_diaries;
	unordered_map<int, diary> m_diaries;
	repository();
	~repository();

	//char* generate_uuid() const;
	int generate_id() const;
	bool check_id_exist(const int id) const;
